...
- **首次启动检查**：新安装会先检查 WebView2、Codex Desktop、Hooks、额度服务、SMTC 播放器与 Obsidian 配置。
...
5. 在首次启动检查或设置的“Codex”页面安装 Hooks，并重启 Codex，即可同步任务开始和完成状态；额度服务可直接复用 Codex Desktop，无需另装 Node.js/npm。
...
Wisland 读取 Windows 的 SMTC 媒体会话，因此播放器需要向系统提供媒体信息。播放过一次的播放器会出现在设置中，可单独调整歌词补偿。若播放器没有被识别，可在“设置 → 音乐 → SMTC 来源”扫描实际应用标识并一键加入白名单。
...
设置、Codex 状态和日志保存在 `%APPDATA%\wisland`。首次连接额度时，Wisland 会把本机 Codex Desktop 自带的 CLI 复制到 `%LOCALAPPDATA%\wisland\runtime\codex-desktop` 后独立调用，不修改 Codex 安装目录。Obsidian 内容仅写入用户指定的本地 Vault；临时文件托盘不上传或复制文件。歌词与 Codex 额度所需的网络请求由对应功能按需发起。
